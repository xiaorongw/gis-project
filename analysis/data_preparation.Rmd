...
hdb = st_read(dsn = "data/geospatial", layer = "hdb_osm_wgs84")
...
```{r}
building_residential <- st_read(dsn='data/geospatial', layer='hdb_osm_extent')
```
```{r}
building_residential <- st_transform(building_residential,3414)
st_crs(building_residential)
```

```{r}
st_bbox(building_residential)
```
